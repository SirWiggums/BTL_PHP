...
// Kiểm tra xem có tham số hang_sp trong URL không
$hang_sp_filter = '';
if (isset($_GET['hang_sp'])) {
    $hang_sp_filter = $_GET['hang_sp'];
    $sql = "SELECT ma_san_pham, ten_san_pham, gia_san_pham, gia_cu, mo_ta_ngan, mo_ta_chi_tiet, so_luong, image 
            FROM sanpham 
            WHERE hang_sp = '$hang_sp_filter'";
} else {
    $sql = "SELECT ma_san_pham, ten_san_pham, gia_san_pham, gia_cu, mo_ta_ngan, mo_ta_chi_tiet, so_luong, image 
            FROM sanpham";
}
...
<body>
    ...
    <h1>
        <?php
            if (!empty($hang_sp_filter)) {
                echo "Sản phẩm thuộc hãng: " . htmlspecialchars($hang_sp_filter);
            } else {
